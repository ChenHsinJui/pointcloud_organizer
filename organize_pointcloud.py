...
class OrganizeBag(object):
    ...
    def echo_bag(self):
        storage_options = rosbag2_py._storage.StorageOptions(
            uri=self._bag, storage_id="sqlite3"
        )
    ...
    def convert_from_byte(self, target_points):
        ...
        for point in target_points:
            ...
            temp = [
                ...
                point[1],
                point[2],
            ]  # x, y, and z are stored in uint8 array format
    ...
    def convert_to_byte(
        self, target_points
    ):  # TODO: encoding may have some issue the output row step will be bigger than before
        data = []
        for point in target_points:
            for ele in point:
    ...
    def rip_out_redundant(self, points):
        ...
        iterator_getter = {
            0: lambda points_array: max(points_array, key=x_getter),
            1: lambda points_array: max(points_array, key=z_getter),
            2: lambda points_array: min(points_array, key=x_getter),
            3: lambda points_array: min(points_array, key=z_getter),
        }
    ...
    def generate_pc_data(self, pc_data, row_step, point_step):
        points_data = [
            pc_data[i : i + point_step] for i in range(0, row_step, point_step)
        ]

        target_data = []
        for point in points_data:
            splitted_point = [
                point[i : i + 4] for i in range(0, len(point), 4)
            ]  # Assume that all field take 4 bytes in size.
            target_data.append(splitted_point)
        assert len(target_data) == int(
            row_step / point_step
        ), "The length of splitted points didn't match the number from row step."

        points = self.convert_from_byte(target_data)
        assert len(points) == len(
            target_data
        ), "The length of decoded points array didn't match the length of the given splitted points arrray."
    ...
    def get_pointcloud(self, unorganized_msg):
        ...
        msg.data, msg.row_step, msg.height = self.generate_pc_data(
            unorganized_msg.data, unorganized_msg.row_step, unorganized_msg.point_step
        )
    ...
    def convert_to_organized(self, output_path):
        ...
        self._writer.create_topic(
            rosbag2_py.TopicMetadata(
                name="/sensing/lidar/bf_lidar/points_raw",
        ...
        while self._reader.has_next():
            ...
            output_msg.header.stamp = Time(
                seconds=t // int(1e9), nanoseconds=t % int(1e9)
            ).to_msg()
            self._writer.write(
                topic_name,
                serialize_message(output_msg),
                t,
            )
        del self._writer
...
def main():
    ...
    parser.add_argument(
        "-e",
        "--echo",
        help="whether you want to echo the bag info or not",
        action="store_true",
    )
    parser.add_argument(
        "-o",
        ...
        type=str,
        required=True,
        help="specify the output path to store",
    )
